(function(exports) {
  ...
  OverTreeInterpreter.prototype = {
    ...
    function DeleteTreeInterpreter(runtime) {
        this.runtime = runtime;
    }

    DeleteTreeInterpreter.prototype = {
        ...
        visit: function(node, value, level) {
            var previous, matched, current, result, first, second, field, left, right, collected, i;
            switch (node.type) {
                case "Field":
                    if (value !== null && isObject(value)) {
                        field = value[node.name];
                    ...
                    return null;
                case "Subexpression":
                    result = this.visit(node.children[0], value, level + 1);
                    for (i = 1; i < node.children.length; i++) {
                        previous = result;
                        result = this.visit(node.children[1], result, level + 1);
                        if (result === null) {
                            return null;
                        }
                        if(level === 0 ) delete previous[node.children[1].name];
                    }
  ...
  // TODO: generalize this so that we don't have to copy all this code
  function remove(data, expression){
    return decorateRemove({})(expression)(data);
  }

    function decorateRemove(fns) {
        var parser = new Parser();
        var runtime = new Runtime();
        Object.assign(runtime.functionTable, fns);
        var interpreter = new DeleteTreeInterpreter(runtime);
        runtime._interpreter = interpreter;
        return function (expression) {
            var node = parser.parse(expression);
            return function (data) {
                interpreter.search(node, data);
